        ...
        for row in range(1, 10):
            for column in range(1, 10):
                if ((row in (1,2,3,7,8,9) and column in (4,5,6)) or (row in (4,5,6) and column in (1,2,3,7,8,9))):
                    ...
                    color='red'
                cell = tkinter.Frame(master, highlightbackground=color, highlightcolor=color, highlightthickness=2, width=50, height=50, padx=3,  pady=3, background='white')
        ...
        for number in range(start, 10):
            if self.validInput(number, boardInput, indexes):
                ...
                if visual:
                    ...
                    self.entries[(indexes[0] * 9 + indexes[1])].configure(foreground="green")
                    self.entries[(indexes[0] * 9 + indexes[1])].insert(0, number)
